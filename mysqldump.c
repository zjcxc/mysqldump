...
// zoujian.zjcxc --> --master-data=3 | 4
// static uint opt_mysql_port= 0, opt_master_data;
static uint opt_mysql_port= 0, opt_master_data=0, opt_master_data_input=0;
// ------------------------------------------------
...
// zoujian.zjcxc --> --all-databases [ignore_db]...
static char **opt_alldbs_argv;
// ------------------------------------------------
...
static struct my_option my_long_options[] =
{
  ...
  //  GET_UINT, OPT_ARG, 0, 0, MYSQL_OPT_MASTER_DATA_COMMENTED_SQL, 0, 0, 0},
   &opt_master_data_input, &opt_master_data_input, 0,
   GET_UINT, OPT_ARG, 0, 0, MYSQL_OPT_MASTER_DATA_COMMENTED_SQL*2, 0, 0, 0},
   // ------------------------------------------------
...
static void short_usage_sub(void)
{
  ...
  printf("OR     %s [OPTIONS] --all-databases [IGNORE_DB1 IGNORE_DB2...] [OPTIONS]\n", my_progname);
  // ------------------------------------------------
}
...
static int get_options(int *argc, char ***argv)
{
  ...
  // zoujian.zjcxc --> --master-data=3 | 4
  if (opt_master_data_input>0){
      opt_master_data = ((opt_master_data_input-1) % MYSQL_OPT_MASTER_DATA_COMMENTED_SQL) + 1;
  }
  // ------------------------------------------------
  ...
  // zoujian.zjcxc --> password from environment variable MYSQLPASSWORD
  else if (!opt_password){
    char *pass=getenv("MYSQLPASSWORD");
    if (pass)
      opt_password=my_strdup(PSI_NOT_INSTRUMENTED, pass, MYF(MY_FAE));
  }
  // zoujian.zjcxc --> user from environment variable MYSQLUSER
  if (!current_user)
    current_user=getenv("MYSQLUSER");
  // ------------------------------------------------
  return(0);
} /* get_options */
...
/* zoujian.zjcxc --> --where='#proc_where', para: dbname, tbname
zj_dump_table_where()
userdefine where for dump_table
*/
...
/* zoujian.zjcxc --> --all-databases [ignore_db]...
zj_opt_alldbs_where()
--all-databases ignore database filter
*/
static void zj_opt_alldbs_where(DYNAMIC_STRING *to){
    int i;    
    for (i=0 ; opt_alldbs_argv[i]!=NULL ; i++){
        char db_name_buff[NAME_LEN*2+3];
        mysql_real_escape_string_quote(
            mysql, db_name_buff,
            opt_alldbs_argv[i], (ulong)strlen(opt_alldbs_argv[i]),
            '\'');
        dynstr_append_checked(to, "'");
        dynstr_append_checked(to, db_name_buff);
        dynstr_append_checked(to, "',");
    }
    dynstr_trunc(to, 1);
    return;
  ...
  if (path)
  {
    ...
    */
             ...
             opt_alldbs)
    {
      ...
      // zoujian.zjcxc --> --all-databases [ignore_db]...
      if (opt_alldbs_argv){
          dynstr_append_checked(&query_string, " AND db NOT IN(");
          zj_opt_alldbs_where(&query_string);
          dynstr_append_checked(&query_string, ")");
      }
      // ------------------------------------------------
    }
...
static int dump_all_tablespaces()
{
  ...
  init_dynamic_string_checked(
      ...
      " WHERE"
      " TABLE_SCHEMA NOT IN (", 128, 64);
  zj_opt_alldbs_where(&where);
...
static int dump_all_databases()
{
  ...
  // zoujian.zjcxc --> --all-databases [ignore_db]...
  DYNAMIC_STRING query_show_dbs;
  init_dynamic_string(&query_show_dbs, "SHOW DATABASES", 32, 64);
   if (opt_alldbs_argv){
       dynstr_append_checked(&query_show_dbs, " WHERE `Database` NOT IN(");
       zj_opt_alldbs_where(&query_show_dbs);
       dynstr_append_checked(&query_show_dbs, ")");
   }
   // ------------------------------------------------
...
//     return 1;
  if (mysql_query_with_error_report(mysql, &tableres, query_show_dbs.str)){
      dynstr_free(&query_show_dbs);
      return 1;
    }
    // ------------------------------------------------
  ...
  // zoujian.zjcxc --> --all-databases [ignore_db]...
  // when ignore myql, dump all dbs saved as part of database_list
  for (; cnt < db_cnt; cnt++){
      if (dump_all_tables_in_db(database_list[cnt]))
        result=1;
      my_free(database_list[cnt]);
  ...
  if (seen_views)
  {
    ...
    //     !(tableres= mysql_store_result(mysql)))
    // {
    if (mysql_query(mysql, query_show_dbs.str) || !(tableres= mysql_store_result(mysql))){
        dynstr_free(&query_show_dbs);
        // ------------------------------------------------
...
static int do_show_master_status(MYSQL *mysql_con)
{
  ...
  else
  {
    ...
    // zoujian.zjcxc --> --master-data=3 | 4
    // else if (!opt_force)
    else if ((!opt_force) && opt_master_data_input <= MYSQL_OPT_MASTER_DATA_COMMENTED_SQL)
    // ------------------------------------------------
    {
